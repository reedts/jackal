...
impl<'a> App<'a> {
    pub fn new(config: &'a Config, cal_view: CalendarView<'a>) -> App<'a> {
        App {
            quit: false,
            views: [
                View::Calendar(Controller::new(&config.key_map, cal_view))
            ],
            active_view: 0,
            config,
        }
...
impl<'a> Widget for App<'a> {
    fn draw(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        Block::default().title("Events").borders(Borders::ALL).draw(layout[1], buf);
    }
}
